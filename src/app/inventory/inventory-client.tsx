...
function Pagination({
  ...
  return (
    <div className="px-4 sm:px-6 py-4 bg-gray-50/50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
...
export function InventoryClient({ initialItems, initialPurchases, initialMovements, userRole }: Props) {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 max-w-7xl mx-auto w-full min-w-0 space-y-8 font-sans">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {isAdmin && (
          <div className="grid grid-cols-2 gap-2 w-full sm:flex sm:flex-wrap sm:gap-3 sm:w-auto">
            <ImportInventoryModal
              triggerClassName="col-span-2 sm:col-span-1 w-full sm:w-auto h-11 px-3 text-xs sm:text-sm font-bold justify-center gap-2 min-w-0 whitespace-normal sm:whitespace-nowrap"
            />
            <AddInventoryItemModal
              triggerClassName="w-full sm:w-auto h-11 px-3 text-xs sm:text-sm font-bold justify-center gap-2 min-w-0 whitespace-normal sm:whitespace-nowrap"
            />
            <AddPurchaseModal
              triggerClassName="w-full sm:w-auto h-11 px-3 sm:px-4 text-xs sm:text-sm font-bold justify-center gap-2 min-w-0 whitespace-normal sm:whitespace-nowrap leading-tight"
            />
          </div>
        )}
      </div>
      ...
      {/* Tabs & Search */}
      <div className="flex flex-col sm:flex-row justify-between gap-4 min-w-0">
        <div className="w-full min-w-0 overflow-x-auto no-scrollbar sm:overflow-visible">
          <div className="flex gap-1 bg-gray-100 dark:bg-gray-800/60 p-1.5 rounded-2xl w-max min-w-full sm:w-auto sm:inline-flex font-cairo">
            {tabs.map(({ key, label, icon: Icon }) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 sm:px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all shrink-0 whitespace-nowrap justify-center ${tab === key
            ...
            {paginatedItems.map(item => (
              <div key={item.id} className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="bg-teal-50 dark:bg-teal-900/20 rounded-xl p-2.5 text-center border border-teal-100 dark:border-teal-900/30">
                ...
                {isAdmin && (
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setEditItem(item)} className="flex-1 min-w-[calc(50%-0.25rem)] flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-teal-600 bg-teal-50 dark:bg-teal-900/20 rounded-xl hover:bg-teal-100 transition">
                      <Pencil size={12} /> Edit
                    </button>
                    {isSuperAdmin && (
                      <button onClick={() => setAdjustItem(item)} className="flex-1 min-w-[calc(50%-0.25rem)] flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-amber-600 bg-amber-50 dark:bg-amber-900/20 rounded-xl hover:bg-amber-100 transition">
                        <SlidersHorizontal size={12} /> Adjust
                      </button>
                    )}
                    <button onClick={() => handleDeactivate(item.id)} className={`flex items-center justify-center gap-1.5 py-2 text-xs font-bold text-red-500 bg-red-50 dark:bg-red-900/20 rounded-xl hover:bg-red-100 transition ${isSuperAdmin ? 'w-full' : 'flex-1 min-w-[calc(50%-0.25rem)]'}`}>
                      <Power size={12} /> Off
                    </button>
                  </div>
                )}
